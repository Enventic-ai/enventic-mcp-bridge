# Enventic MCP User Guide (Windows)

From install to daily use · Claude Desktop / Claude Code · Windows · 5-minute setup

> **On a Mac?** Use `GUIDE.md` instead — the config paths, quit gesture, and
> shell commands differ. This file is the Windows edition.
...
|---|---|
| Claude Desktop *or* Claude Code | Download from claude.ai, skip if already installed |
| Python 3.10+ | Install from [python.org](https://www.python.org/downloads/windows/) **or** `winget install Python.Python.3.12`. **Tick "Add python.exe to PATH"** in the installer. Check with `python --version` in a new terminal. |
| Enventic account (real, not demo) | Sign up at `https://www.enventic.ai` |
| Git | [git-scm.com/download/win](https://git-scm.com/download/win) **or** `winget install Git.Git` |

> **Terminal.** Use **PowerShell** (Start → type "PowerShell") or Windows
> Terminal for every command in this guide.

---

## 4. Step 1 — Install the bridge

Clone the bridge repo (public, ~30 seconds). It lands in your user folder:

```powershell
git clone https://github.com/Enventic-ai/enventic-mcp-bridge.git "$env:USERPROFILE\enventic-mcp-bridge"
```

Installed layout (e.g. `C:\Users\you\enventic-mcp-bridge\`):

```
enventic-mcp-bridge\
  server.py    # stdio bridge (stdlib only)
  README.md
  GUIDE.md            # macOS guide
  GUIDE-Windows.md    # this guide
  .gitignore
```
...
> **Windows JSON paths need double backslashes.** In the config file write
> `C:\\Users\\you\\enventic-mcp-bridge\\server.py` (each `\` doubled), or
> use forward slashes `C:/Users/you/enventic-mcp-bridge/server.py`. A single
> backslash is a JSON escape and will break the file.

### Claude Desktop (classic chat app)

Config file lives at `%APPDATA%\Claude\claude_desktop_config.json`. Open it:

```powershell
notepad "$env:APPDATA\Claude\claude_desktop_config.json"
```

If Notepad offers to create a new file, say yes. Paste the block below
(replace `you` with your Windows username and paste your token):
...
```json
{
  "mcpServers": {
    "enventic": {
      "command": "python",
      "args": ["C:\\Users\\you\\enventic-mcp-bridge\\server.py"],
      "env": {
...
Config file: `%USERPROFILE%\.claude.json`. Open it:
...
> **`python` vs `py`.** If `command: "python"` fails to start, either use the
> full path to `python.exe` (find it with `where python` in PowerShell) or
> the launcher: `"command": "py"`, `"args": ["-3", "C:\\Users\\you\\..."]`.

---

## 7. Step 4 — Restart & verify

1. **Fully quit Claude** — closing the window is not enough on Windows.
   Right-click the Claude icon in the **system tray** (bottom-right, near
   the clock; click the `^` to reveal hidden icons) → **Quit**. If there is
   no tray icon, end `Claude.exe` from Task Manager (Ctrl+Shift+Esc).
2. Reopen Claude.
3. Verify with any of the three methods:
   - **Ask directly**: "Use enventic to list what data is missing for CSRD 2024." — should return a list of items.
   - **Input-box `+` / attachment button** — the menu should show `enventic`.
   - **Type `/mcp`** — the panel should show `enventic ✓ Connected`.

> **Common reasons it didn't take effect**:
> - Didn't fully quit — closing the window leaves it running in the tray; Quit from the tray or Task Manager
> - Wrong path — `args` must be an absolute Windows path with **double backslashes** (or forward slashes)
> - Python not on PATH — run `where python` in PowerShell; if nothing prints, reinstall Python with "Add to PATH" ticked, or use the full `python.exe` path in `command`
...
→ Claude calls `list_required_inputs` and returns the missing items grouped by domain A–H with priority guidance.
...
| MCP shows grey / "failed" in Claude Desktop | Bridge failed to start | Check the logs at `%APPDATA%\Claude\logs` (open with `explorer "$env:APPDATA\Claude\logs"`) — usually `python` not found or a path typo |
...
| Tool call returns "not found" | Claude wasn't fully restarted | Quit from the system tray (or Task Manager), then reopen |
| `python` not found / not recognised | Not on PATH | Run `where python`; if empty, reinstall Python with "Add to PATH" ticked, or put the full `python.exe` path in `command`, or use `"command": "py"` |
| Config file won't parse | Single backslash in a path | Use `\\` (double) or `/` (forward) in JSON `args` |
...
### Do
- Keep the config file inside your user profile (`%APPDATA%` / `%USERPROFILE%`); on a shared PC restrict it to your account:
  ```powershell
...
*Enventic MCP Phase 1 · Windows edition · Generated 2026-09-03 (28 tools · Process #3 Unlock) · Internal use*
